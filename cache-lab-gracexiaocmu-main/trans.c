...
/**
 * This transpose function uses blocking of 8*8 blocks,
 * and it stores diagonal elements of matrix A into tmp first
 * and then stores them to matrix B to reduce evictions
 */
static void trans_small5(size_t M, size_t N, double A[N][M], double B[M][N],
                         double tmp[TMPCOUNT]) {
    size_t jump = 8;
    size_t offset = 8;

    for (size_t col = 0; col < M; col += jump) {
        for (size_t row = 0; row < N; row += jump) {
            for (size_t r = row; r < row + jump; r++) {
                for (size_t c = col; c < col + jump; c++) {
                    if (r < N && c < M) {
                        if (r != c) {
                            B[c][r] = A[r][c];
                        } else {
                            tmp[(r + offset) % TMPCOUNT] = A[r][c];
                        }
                    }
                }
                if (col == row && r < N && r < M) {
                    B[r][r] = tmp[(r + offset) % TMPCOUNT];
                }
            }
        }
    }
...
void registerFunctions(void) {
    ...
    registerTransFunction(trans_small5, "For 32*32 v5");
}
